...
- **Case studies** (`/use-cases`, `/use-cases/:id`) — real, publicly-documented AWS GenAI deployments. Most are cards; where enough has been published, a **deep dive** reconstructs the architecture with synced diagrams and draw.io exports. Every claim is typed with its provenance — `documented` claims link the source, `inferred` claims state what they reason from, and a node on a documented diagram gets an AWS logo only if that service was actually named.
